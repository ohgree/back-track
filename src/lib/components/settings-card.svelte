...
<div class="glass rounded-2xl border border-white/10 overflow-hidden">
	...
	{#if showSettings}
		<div class="p-4 space-y-4 border-t border-white/10">
			...
			<div class="flex items-center justify-between">
				...
				<div class="flex items-center gap-1">
					<button
						...
						class="px-3 py-1.5 text-sm rounded-lg transition-colors {i18n.locale === 'en' ? 'bg-back-500/30 text-back-400' : 'bg-white/10 text-white/60 hover:bg-white/20'}"
					>
						{i18n.t('english')}
					</button>
					<button
						onclick={() => setLocale('ko')}
						class="px-3 py-1.5 text-sm rounded-lg transition-colors {i18n.locale === 'ko' ? 'bg-back-500/30 text-back-400' : 'bg-white/10 text-white/60 hover:bg-white/20'}"
					>
						{i18n.t('korean')}
					</button>
				</div>
			...
			<div class="space-y-4">
				<p class="text-white/60 text-xs uppercase tracking-wider font-display">{i18n.t('sensitivity')}</p>
